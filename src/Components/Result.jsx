...
export default function Result({ weatherData, historyData , historySearch }) {
    ...
    const historyItems = historyData.map(
        (item, index) => {
            return <li onClick={() => historySearch(item)} className='text-xl cursor-pointer ml-3' key={index}>{item}</li>
        }
    )
    return (
        <div className='grid grid-cols-4 shadow-xl rounded-xl background mt-5 p-2'>
            <div className="col-span-1 border-r-2">
                <span className='text-center block text-xl font-bold'>History</span>
                <ul>
                    {historyItems}
                </ul>
            </div>
